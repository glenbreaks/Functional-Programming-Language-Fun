...
program xs1 = do
    ...
    case xs2 of
        Semicolon : xs3 -> do
            ...
            return (foldl ProgramX e es, xs4)
        _ -> Nothing
...
def xs1 = do
    (e, xs2)  <- variable xs1
    (es, xs3) <- restDef xs2
    return (foldl DefX e es, xs3)

restDef :: Parser [Expression]
restDef (Name i : xs) = restDef xs
...
locDefs xs1 = do
    ...
    return (foldl LocDefsX e es, xs3)
...
expr :: Parser Expression -- sollte eig Expression sein, aber so wegen restDef!!
expr (Let : xs1) = do
    (e, xs2) <- locDefs xs1
    case xs2 of
        In : xs3 -> do
...
negExpr (Minus : xs1) = do
    (e, xs2)  <- addExpr xs1
    return (NegX e, xs2)
negExpr xs1          = do       -- addExpr ausgeschrieben
    (e, xs2)  <- multExpr xs1
    (es, xs3) <- restAddExpr xs2
